...
BEHAVIOR_SPECS: tuple[BehaviorSpec, ...] = (
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"send money",
            r"upi|neft|imps|rtgs",
            r"bank details",
            r"account number",
            r"card (?:number|details|cvv)",
            r"due amount",
            r"outstanding",
            r"bill",
            r"fee",
    ...
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"credentials)",
            r"enter (?:your|the)? ?(?:password|passcode|pin|otp|" r"code|credentials)",
            r"send (?:me|us|your|the)? ?(?:password|passcode|" r"pin|otp|code)",
            r"provide (?:your|the)? ?(?:password|passcode|pin|"
            r"otp|code|credentials)",
            r"tell (?:me|us) (?:your|the)? ?(?:otp|" r"password|code|pin)",
            r"what is your (?:otp|password|code|pin)",
            r"forward (?:me|the|this|your)? ?(?:otp|code)",
            r"give (?:me|us) " r"(?:the|your)? ?(?:otp|code|password)",
            r"confirm (?:your )?password",
            r"verify your (?:otp|password)",
            r"re-?enter your (?:otp|code)",
            r"one[- ]?time password",
        ),
        gate=_requestive,
        boost=lambda ctx: (
            0.05 if ctx.semantic.semantic_features.has_credential_request else 0.0
        ),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            r"login to (?:your|the)",
            r"log in (?:to|with|using)",
            r"sign-?in to",
            r"sign in to",
            r"login here",
            r"authenticate (?:yourself|your)",
            r"enter (?:your )?credentials",
            r"verify (?:your )?(?:login|" r"credentials|identity)",
            r"session (?:expired|timed out)",
            r"two-?factor",
            r"2[- ]?fa\b",
            r"biometric",
        ),
    ...
    BehaviorSpec(
        ...
        _any_pat(
            r"kyc\b",
            r"identity (?:verification|proof)",
            r"verify (?:your )?" r"(?:identity|id)",
            r"aadhaar",
            r"pan card",
            r"proof of (?:identity|" r"address)",
            r"selfie",
    ...
    BehaviorSpec(
        ...
        _any_pat(
            r"click",
            r"tap (?:here|this)",
            r"open (?:the|this|our)? ?(?:link|url|" r"website|page)",
            r"visit (?:our|the)? ?(?:website|site|link)",
            r"follow (?:the|this) link",
            r"go to (?:our|this|the)",
            r"redirect",
            r"scanned? (?:this )?qr",
        ),
        gate=_has_url,
        boost=lambda ctx: 0.08 if _has_url(ctx) else 0.0,
    ),
    BehaviorSpec(
        "Information Collection",
        _any_pat(
            r"share (?:your|some|more) (?:details|information)",
            r"tell (?:us|me)" r" (?:about|your)",
            r"enter (?:your )?(?:details|information)",
            r"fill (?:out|in)? (?:the|this)? ?(?:form|details)",
            r"provide (?:your )?" r"details",
            r"update (?:your )?details",
            r"what is your",
            r"your (?:name|age|address|number)",
        ),
        gate=lambda ctx: _has_questions(ctx) or _requestive(ctx),
        boost=lambda ctx: (
            0.05 if ctx.semantic.semantic_features.question_count else 0.0
        ),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"get back to (?:me|us)",
            r"will you be",
            r"are you (?:free|available|" r"coming|interested)",
            r"want to (?:meet|talk|chat)",
            r"let's (?:meet|" r"talk|catch up|chat)",
            r"reply (?:back )?(?:soon|asap|to this)",
        ),
        gate=lambda ctx: _has_questions(ctx) or _requestive(ctx),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"reward\w*",
            r"promo\w*",
            r"free (?:gift|trial|sample|shipping)",
    ...
    BehaviorSpec(
        ...
        _any_pat(
            r"introducing",
            r"discover",
            r"new (?:product|arrival|launch|range)",
            r"our (?:product|brand|collection)",
            r"visit our (?:website|store)",
            r"showroom",
            r"book (?:your|a)? ?(?:demo|slot|consultation)",
            r"featured",
            r"our latest",
            r"launch\w*",
        ),
    ...
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"book(?:ing)?",
            r"schedule",
            r"visit (?:us|our " r"(?:clinic|office|store))",
            r"consultation",
            r"check-?up",
            r"slot",
            r"reschedule",
            r"confirm (?:your )?(?:appointment|visit)",
    ...
    BehaviorSpec(
        ...
        _any_pat(
            r"reminder",
            r"due (?:date|payment|amount|tomorrow)",
            r"renewal",
            r"expir\w*",
            r"overdue",
            r"coming (?:due|up)",
            r"auto-?debit",
            r"payable (?:on|by|before)",
            r"valid (?:until|till|upto)",
            r"last (?:date|day) to",
            r"don't forget",
        ),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"technical (?:issue|support)",
            r"troubleshoot",
            r"refund (?:request|issue)",
            r"unable to",
        ),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"helpline",
            r"toll-?free",
            r"service (?:center|centre|desk)",
            r"executive",
            r"our team",
            r"care team",
            r"complaint (?:number|cell)",
            r"we value (?:you|our " r"customers)",
            r"your satisfaction",
        ),
    ),
    BehaviorSpec(
        ...
        _any_pat(
            ...
            r"family",
            r"your weekend",
            r"weekend (?:plans|trip|party|hangout)",
            r"met (?:you|him|her)",
            r"let's (?:meet|catch up|hang out)",
            r"how have you been",
            r"how is life",
            r"miss(?:ed)? you",
            r"wanted to (?:talk|chat)",
        ),
    ),
)
